...
Resolution Phase (Order is vital):  
1. Steal (Hands) -> 2. Ranged (Arrows vs Shields) -> 3. Melee (Axes vs Helmets).  
Game Modes: - 2-Player: Local pass-and-play.  
Single Player: Player vs. an AI Bot that prioritizes dice based on its class (e.g., Berserker Bot hunts for Axes).   
